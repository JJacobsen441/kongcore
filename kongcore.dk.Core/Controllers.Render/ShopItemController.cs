...
{
    public class ShopItemController : Umbraco.Web.Mvc.RenderMvcController
    {
        ContentHelper helper;

        public ShopItemController() : base()
        {
            //root = new Root(CurrentPage);
        }

        // Any request for the 'ProductAmpPage' template will be handled by this Action
        public ActionResult ShopItem(ContentModel model)
        {
            try
            {
                ...
                DTO_ShopItem dto = biz_shop.ToDTO(helper);
